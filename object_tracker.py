...
def resizeAndMaskVideo (vid):
    ...
    while True:
        ...
        if ret == True:
            ...
            imgsize = np.zeros(b.shape[:2], dtype='uint8')
            pts2 = np.array([[0,0],[0,40],[200,40],[200,20],[310,20],[310,0]])
            y = cv2.drawContours(imgsize, [pts2], -1, (255, 255, 255), -1)
            pts = np.array([[320, 0],[315, 20], [315, 360], [540, 360],[555,240], [400, 50],[350,30], [350,0]])
            w = cv2.drawContours(imgsize, [pts], -1, (255, 255, 255), -1)
            final = cv2.bitwise_and(b, b, mask=w)
            final = cv2.bitwise_and(b, b, mask=y)
            out.write(final)
        else:
            break
